...
<body>
				...
				<td class="commentInfo">
					<div>
						...
						<div>
							...
							<span onclick="editComment(${cdto.seq});">📝</span>
						</div>
						</c:if>
					</div>
				</td>
	...
	<script>
		...
		function delComment(seq) {
			...
			if (confirm('delete?')) {
				location.href 
				= '/semi/board/delcomment.do?seq=' + seq + '&bseq=${dto.seq}';
			}
			...
			let editRow = `
				<tr class="commentEditRow">
					...
					<td class="commentEdit">
						...
						<button type="button" 
							onclick="editOkComment(\${seq});">수정하기</button>
					</td>
				</tr>
			`;
		...
		function editOkComment(seq) {
			...
			const content = $('input[type=text][name=content]').val(); //수정한 댓글
			//const seq = 댓글 번호;
			const bseq = ${dto.seq};
