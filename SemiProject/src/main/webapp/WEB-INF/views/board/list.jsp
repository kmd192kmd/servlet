...
<body>
	...
	<div id="main">
		<h1>
			...
			<small style="color: cornflowerblue;">검색보기</small>
			</c:if>
		...
		<c:if test="${map.search == 'y'}">
		<div id="labelSearch">
			'${map.word}'(으)로 검색한 결과 ${list.size()}개 게시물이 있습니다.
		</div>
		</c:if>
		...
		<div id="pagebar">
			...
			<input type="button" value="이동하기" id="btnPage"
	onclick="location.href='/semi/board/list.do?page=' + $('#page').val();">
		</div> 
		--%>
		...
		<div>
			...
			<button type="button" class="add primary"
				onclick="location.href='/semi/board/add.do';">쓰기</button>
			</c:if>
		</div>
